...
@pytest.mark.offline
@mock_aws
def test_local__logs_retention_policy():
    logs = _logs()

    clean = "/workshop/clean/local"
    logs.create_log_group(logGroupName=clean)
    logs.put_retention_policy(logGroupName=clean, retentionInDays=30)

    messy = "/workshop/messy/local"
    logs.create_log_group(logGroupName=messy)

    lg_clean = [g for g in logs.describe_log_groups(logGroupNamePrefix="/workshop/clean")["logGroups"] if g["logGroupName"] == clean][0]
    assert lg_clean.get("retentionInDays", 0) >= 30

    lg_messy = [g for g in logs.describe_log_groups(logGroupNamePrefix="/workshop/messy")["logGroups"] if g["logGroupName"] == clean][0]
    assert "RetentionInDays" not in lg_messy